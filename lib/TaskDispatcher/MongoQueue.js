...
module.exports = class MongoQueue {
  ...
  async doLoop () {
    ...
    const queryLock = await this.lock(queryLockKey, Number(env['WORKER_MONGO_QUERY_TIMEOUT']))
    if (queryLock) {
      try {
      ...
      $aggregate: [
        {
          $match: {
            $or: [
              ...
              { status: 'executing', executingTime: { $lt: now - Number(env['WORKER_TASK_DEFAULT_TIMEOUT']) } }
            ]
          }
        }, {
