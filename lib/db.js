...
getDbs = () => {
  ...
  if (process.env['REDIS_URL']) {
    ...
    if (fs.existsSync(ROOT_PATH + '/config/' + redisOpts['key'])) {
      tls = {
        sslKey: fs.readFileSync(ROOT_PATH + '/config/' + redisOpts['key']),
        sslCert: fs.readFileSync(ROOT_PATH + '/config/' + redisOpts['cert']),
        sslCA: fs.readFileSync(ROOT_PATH + '/config/' + redisOpts['ca'])
      }
    }

    redis1 = new Redis({
      sentinels: redisOpts['sentinels'],
      sslPort: redisOpts['ssl_port'],
      tls: tls,
      name: 'master',
      db: redisOpts['db'] || 0
    })
